    ...
    for batch in tqdm(training_loader, mininterval=2,
        ...
        if opt.decoder == 'attention':
            if opt.pretrain and epoch < 1:
                outputs = model(batch_x, batch_x_mark, dec_inp, batch_y_mark, batch_y, True)
                batch_y = torch.cat([batch_x, batch_y], dim=1)
            else:
                outputs = model(batch_x, batch_x_mark, dec_inp, batch_y_mark, batch_y, False)
        elif opt.decoder == 'FC':
...
def eval_epoch(model, test_dataset, test_loader, opt, epoch):
    ...
    mae, mse, rmse, mape, mspe, nse = metric(preds_extracted[:, 0], trues_extracted[:, 0])
    print(nse)
    mae, mse, rmse, mape, mspe, nse = metric(preds_extracted[:, 1], trues_extracted[:, 1])
    print(nse)
    mae, mse, rmse, mape, mspe, nse = metric(preds_extracted[:, 2], trues_extracted[:, 2])
    print(nse)
    mae, mse, rmse, mape, mspe, nse = metric(preds_extracted[:, 3], trues_extracted[:, 3])
    print(nse)
    mae, mse, rmse, mape, mspe, nse = metric(preds_extracted[:, 4], trues_extracted[:, 4])
    print(nse)
    mae, mse, rmse, mape, mspe, nse = metric(preds_extracted[:, 5], trues_extracted[:, 5])
    print(nse)
    mae, mse, rmse, mape, mspe, nse = metric(preds_extracted[:, 6], trues_extracted[:, 6])
    print(nse)
    mae, mse, rmse, mape, mspe, nse = metric(preds_extracted, trues_extracted)
...
def train(model, optimizer, scheduler, opt, model_save_dir):
    ...
    for epoch_i in range(opt.epoch):
        ...
        mse, mae, rmse, mape, mspe, nse = eval_epoch(model, test_dataset, test_dataloader, opt, epoch_i)
