...
def eval_epoch(model, validation_data, opt):
    ...
    total_label = 0
    total_pred_num = 0
    total_sse = 0  # Sum of squared errors for NSE
    labels_list = []  # List to store all labels for NSE denominator calculation
    index = 0
    criterion = LossFactory(opt.ignore_zero)
    with torch.no_grad():
        for batch in tqdm(validation_data, mininterval=2,
            ...
            # 更新NSE计算所需的变量
            total_sse += torch.sum((mu_pre - label) ** 2).detach().item()
    ...
    # 计算观测值的平均值
    mean_observed = np.mean(labels_list)
    # 计算观测值与其平均值之差的平方和
    total_sst = np.sum((np.array(labels_list) - mean_observed) ** 2)
    # 计算NSE
    nse = 1 - (total_sse / total_sst)

    return total_likelihood / total_pred_num, se, ae, nse
...
def train(model, optimizer, scheduler, opt, model_save_dir):
    ...
    for epoch_i in range(opt.epoch):
        ...
        start = time.time()
        valid_likelihood, valid_mse, valid_mae, valid_nse = eval_epoch(model, validation_data, opt)
        print('  - (Testing)     loglikelihood: {ll: 8.5f}, '
              'RMSE: {RMSE: 8.5f}, '
              'NMAE: {accuracy: 8.5f}, '
              'NSE: {NSE: 8.5f}, '
              'elapse: {elapse:3.3f} min'
              .format(ll=valid_likelihood, RMSE=valid_mse, accuracy=valid_mae, NSE=valid_nse,
